...
impl Evaluator {
    ...
    pub fn evaluate(&self) -> f64 {
        ...
        self.rpn_tokens.iter().for_each(|token| match token {
            Token::LITERAL(value) => stack.push(value.trim().parse::<f64>().expect("")),
            Token::PLUS => {
                let b = stack.pop().expect("Cannot pop out of stack");
                let a = stack.pop().expect("Cannot pop out of stack");
                stack.push(a + b)
            }
            Token::MINUS => {
                let b: f64 = stack.pop().expect("Cannot pop out of stack");
                let a: f64 = stack.pop().expect("Cannot pop out of stack");
                stack.push(a - b);
            }
            Token::MUTIPLY => {
                let b = stack.pop().expect("Cannot pop out of stack");
